...
function addPost ($mysqli, $data) {
	$title = $data['title'];
	$p_date = $data['p_date'];
	$post = $mysqli->query("INSERT INTO posts (title, p_date) VALUES ('$title','$p_date')");
...
function updatePost($mysqli, $id, $data) {
	$title = $data['title'];
	$p_date = $data['p_date'];
	// $mysqli->query("UPDATE posts SET 'title', 'p_date' VALUES '$title', '$p_date' WHERE 'posts'.'id' = '$id'"); 
	$mysqli->query("UPDATE `posts` SET `title` = '$title', `p_date` = '$p_date' WHERE `posts`.`id` = '$id'"); 
